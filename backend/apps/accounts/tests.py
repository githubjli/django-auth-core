...
class MembershipAPITestCase(APITestCase):
    ...
    def test_manual_payment_info_returns_read_only_plan_payment_details_without_order(self):
        ...
        self.assertEqual(response.data['currency'], TOKEN_SYMBOL)
        self.assertIn(f'exact {TOKEN_SYMBOL} amount', response.data['notice'])
        legacy_notice_text = 'LBC' + ' amount'
        self.assertNotIn(legacy_notice_text, response.data['notice'])
...
class WalletPrototypeAPITestCase(APITestCase):
    ...
    def test_buyer_can_submit_product_order_wallet_payment_without_marking_paid(
        ...
        self.assertEqual(response.data['confirmations'], 0)
    ...
    def test_duplicate_product_payment_does_not_trigger_second_wallet_send(self, mock_unlock, mock_send, mock_lock):
        ...
        self.assertEqual(second.data['confirmations'], 0)
    ...
    @patch('apps.accounts.services.LbryDaemonClient.wallet_send')
    @patch('apps.accounts.services.LbryDaemonClient.wallet_unlock')
    def test_product_payment_existing_txid_skips_unlock_and_send(self, mock_unlock, mock_send, mock_lock):
        buyer = self.create_user('wallet-product-existing-txid@example.com')
        buyer.linked_wallet_id = 'wallet-main'
        buyer.save(update_fields=['linked_wallet_id'])
        _, _, product = self.create_store_product(slug='wallet-product-store-existing-txid')
        shipping = self.create_shipping_address(buyer)
        product_order, payment_order = self.create_product_order(buyer, product, shipping)
        payment_order.txid = 'tx-existing-001'
        payment_order.confirmations = 2
        payment_order.save(update_fields=['txid', 'confirmations', 'updated_at'])
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['detail'], 'Payment already submitted and is waiting for confirmation.')
        self.assertEqual(response.data['txid'], 'tx-existing-001')
        self.assertEqual(response.data['payment_order_status'], PaymentOrder.STATUS_PENDING)
        self.assertEqual(response.data['product_order_status'], ProductOrder.STATUS_PENDING_PAYMENT)
        self.assertEqual(response.data['confirmations'], 2)
        mock_unlock.assert_not_called()
        mock_send.assert_not_called()
        mock_lock.assert_not_called()
    ...
    def test_product_insufficient_funds_returns_clear_detail(self, mock_unlock, mock_send, mock_lock):
        ...
        self.assertEqual(response.status_code, status.HTTP_502_BAD_GATEWAY)
        self.assertIn('Buyer wallet does not have enough spendable funds', response.data['detail'])
        self.assertIn('required_amount=4.50000000', response.data['detail'])
        self.assertIn('wallet_id=wallet-main', response.data['detail'])
...
class ProductOrderFlowAPITestCase(APITestCase):
    ...
    def test_product_order_creation_uses_configured_product_receive_address(self):
        ...
        self.assertEqual(response.data['qr_payload']['t'], 'product_payment')
        self.assertEqual(response.data['qr_payload']['o'], response.data['order_no'])
        self.assertNotIn('pay_to_address', response.data['qr_payload'])
    ...
    def test_product_order_creation_without_fixed_address_calls_daemon(self, mock_address_unused):
        ...
        self.assertEqual(response.data['qr_payload']['t'], 'product_payment')
        self.assertEqual(response.data['qr_payload']['o'], order.order_no)
    ...
    def test_qr_payload_is_short_signed_reference(self):
        ...
        self.assertEqual(set(payload.keys()), {'v', 't', 'o', 's'})
        self.assertEqual(payload['v'], 1)
        self.assertEqual(payload['t'], 'product_payment')
        self.assertTrue(payload['s'])

    def test_payment_qr_resolve_valid_short_payload(self):
        buyer = self.create_user('qr-resolve-valid@example.com')
        _, _, product = self.create_store_product(slug='store-qr-resolve-valid')
        address = self.create_shipping_address(buyer)
        order, response = self.create_product_order(buyer, product, address)
        payload = response.data['qr_payload']
        expires_at_iso = order.payment_order.expires_at.isoformat() if order.payment_order.expires_at else ''
        expected_amount = str(order.payment_order.expected_amount_lbc if order.payment_order.expected_amount_lbc is not None else order.total_amount)
        expected_sig = sign_product_qr_payload(
            version=1,
            payload_type='product_payment',
            order_no=order.order_no,
            pay_to_address=order.payment_order.pay_to_address or '',
            expected_amount=expected_amount,
            currency=order.currency,
            expires_at=expires_at_iso,
        )
        self.assertEqual(payload['s'], expected_sig)

        resolve_response = self.client.post(
            reverse('payment-qr-resolve'),
            {'payload': response.data['qr_text']},
            format='json',
        )
        self.assertEqual(resolve_response.status_code, status.HTTP_200_OK, resolve_response.data)
        self.assertEqual(resolve_response.data['type'], 'product_payment')
        self.assertEqual(resolve_response.data['order_no'], order.order_no)
        self.assertEqual(resolve_response.data['pay_to_address'], order.payment_order.pay_to_address)
        self.assertEqual(resolve_response.data['payment_status'], order.payment_order.status)
        self.assertNotIn('shipping_address_snapshot', resolve_response.data)
        self.assertNotIn('wallet_id', resolve_response.data)
        self.assertEqual(payload['o'], order.order_no)

    def test_payment_qr_resolve_supports_legacy_full_payload(self):
        buyer = self.create_user('qr-resolve-legacy@example.com')
        _, _, product = self.create_store_product(slug='store-qr-resolve-legacy')
        address = self.create_shipping_address(buyer)
        ...
        legacy_payload['sig'] = sign_product_qr_payload(
            version=legacy_payload['version'],
            payload_type=legacy_payload['type'],
            order_no=legacy_payload['order_no'],
            pay_to_address=legacy_payload['pay_to_address'],
            expected_amount=legacy_payload['expected_amount'],
            currency=legacy_payload['currency'],
            expires_at=legacy_payload['expires_at'],
        )
        response = self.client.post(
            reverse('payment-qr-resolve'),
            {'payload': json.dumps(legacy_payload, separators=(',', ':'))},
            format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['order_no'], order.order_no)

    def test_payment_qr_resolve_invalid_signature_rejected(self):
        buyer = self.create_user('qr-tamper@example.com')
        _, _, product = self.create_store_product(slug='store-qr-tamper')
        address = self.create_shipping_address(buyer)
        order, response = self.create_product_order(buyer, product, address)
        payload = response.data['qr_payload']
        tampered_payload = {'v': 1, 't': 'product_payment', 'o': order.order_no, 's': payload['s'][:-1] + '0'}
        response_invalid = self.client.post(
            reverse('payment-qr-resolve'),
            {'payload': json.dumps(tampered_payload, separators=(',', ':'))},
            format='json',
        )
        self.assertEqual(response_invalid.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Invalid QR signature', response_invalid.data['detail'])

    def test_payment_qr_resolve_rejects_expired_order(self):
        buyer = self.create_user('qr-expired@example.com')
        _, _, product = self.create_store_product(slug='store-qr-expired')
        address = self.create_shipping_address(buyer)
        order, response = self.create_product_order(buyer, product, address)
        order.payment_order.expires_at = django_timezone.now() - timedelta(minutes=1)
        order.payment_order.save(update_fields=['expires_at', 'updated_at'])
    ...
    def test_payment_qr_resolve_rejects_unsupported_type(self):
        payload = {'v': 1, 't': 'membership_payment', 'o': 'POX', 's': 'abc'}
        response = self.client.post(
            reverse('payment-qr-resolve'),
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Unsupported QR payload type', response.data['detail'])
    ...
    def test_buyer_cannot_confirm_received_before_shipment(self):
        buyer = self.create_user('buyer-receive-before-ship@example.com')
        seller, _, product = self.create_store_product('seller-receive-before-ship@example.com', slug='store-receive-before-ship')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)

        self.client.force_authenticate(user=buyer)
        response = self.client.post(reverse('product-order-confirm-received', args=[order.order_no]), format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Only shipping orders can be confirmed as received', response.data['detail'])
        order.refresh_from_db()
        self.assertEqual(order.status, ProductOrder.STATUS_PENDING_PAYMENT)
        self.assertIsNone(order.completed_at)
        self.assertFalse(SellerPayout.objects.filter(product_order=order).exists())

    def test_buyer_cannot_confirm_received_twice_after_completed(self):
        admin = self.create_user('admin-receive-twice@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-receive-twice@example.com')
        seller, _, product = self.create_store_product('seller-receive-twice@example.com', slug='store-receive-twice')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        ...
        self.client.post(
            reverse('seller-product-order-ship', args=[order.order_no]),
            {'carrier': 'Thailand Post', 'tracking_number': 'TH-TWICE-1'},
            format='json',
        )

        self.client.force_authenticate(user=buyer)
        first = self.client.post(reverse('product-order-confirm-received', args=[order.order_no]), format='json')
        self.assertEqual(first.status_code, status.HTTP_200_OK)
        second = self.client.post(reverse('product-order-confirm-received', args=[order.order_no]), format='json')
        self.assertEqual(second.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Only shipping orders can be confirmed as received', second.data['detail'])

    def test_seller_cannot_confirm_received_through_buyer_endpoint(self):
        admin = self.create_user('admin-receive-seller-block@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-receive-seller-block@example.com')
        seller, _, product = self.create_store_product('seller-receive-seller-block@example.com', slug='store-receive-seller-block')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        ...
        self.client.post(
            reverse('seller-product-order-ship', args=[order.order_no]),
            {'carrier': 'Thailand Post', 'tracking_number': 'TH-SELLER-BLOCK-1'},
            format='json',
        )
        response = self.client.post(reverse('product-order-confirm-received', args=[order.order_no]), format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_admin_mark_settled_updates_payout_and_order(self):
        ...
        self.assertEqual(payout.payout_address, 'seller-address-1')
        self.assertEqual(payout.note, 'manual settlement')
        self.assertIsNotNone(payout.paid_at)
        self.assertIsNotNone(order.settled_at)
        self.assertEqual(order.payment_order.status, PaymentOrder.STATUS_PAID)

    def test_non_admin_cannot_mark_order_settled(self):
        buyer = self.create_user('nonadmin-settle-buyer@example.com')
        seller, _, product = self.create_store_product('nonadmin-settle-seller@example.com', slug='store-nonadmin-settle')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        ...
        response = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-nonadmin-1'},
            format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_admin_cannot_settle_non_completed_order_statuses(self):
        admin = self.create_user('admin-settle-invalid@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-settle-invalid@example.com')
        seller, _, product = self.create_store_product('seller-settle-invalid@example.com', slug='store-settle-invalid')
        address = self.create_shipping_address(buyer)
        statuses_to_reject = [
            ProductOrder.STATUS_PENDING_PAYMENT,
            ProductOrder.STATUS_PAID,
            ProductOrder.STATUS_SHIPPING,
            ProductOrder.STATUS_CANCELLED,
        ]
        for idx, current_status in enumerate(statuses_to_reject, start=1):
            order, _ = self.create_product_order(buyer, product, address)
            ...
            response = self.client.post(
                reverse('admin-product-order-mark-settled', args=[order.order_no]),
                {'txid': f'settle-invalid-{idx}'},
                format='json',
            )
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
            self.assertIn('Only completed orders can be settled', response.data['detail'])
            order.refresh_from_db()
            self.assertEqual(order.status, current_status)
            self.assertIsNone(order.settled_at)

    def test_admin_cannot_settle_completed_order_with_active_refund_request(self):
        admin = self.create_user('admin-settle-active-refund@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-settle-active-refund@example.com')
        _, _, product = self.create_store_product('seller-settle-active-refund@example.com', slug='store-settle-active-refund')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        order.status = ProductOrder.STATUS_COMPLETED
        order.completed_at = datetime.now(timezone.utc)
        ...
        ProductRefundRequest.objects.create(
            ...
            currency='THB-LTT',
            status=ProductRefundRequest.STATUS_REQUESTED,
        )

        self.client.force_authenticate(user=admin)
        response = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-active-refund-1'},
            format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('active refund request', response.data['detail'])

    @patch('apps.accounts.services.LbryDaemonClient.wallet_send')
    def test_admin_manual_settlement_does_not_call_wallet_send(self, mock_wallet_send):
        admin = self.create_user('admin-settle-nowalletsend@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-settle-nowalletsend@example.com')
        _, _, product = self.create_store_product('seller-settle-nowalletsend@example.com', slug='store-settle-nowalletsend')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        order.status = ProductOrder.STATUS_COMPLETED
        order.completed_at = datetime.now(timezone.utc)
        ...
        response = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-nowalletsend-1', 'payout_address': 'seller-nowalletsend-address'},
            format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_wallet_send.assert_not_called()

    def test_duplicate_admin_settlement_is_rejected(self):
        admin = self.create_user('admin-settle-duplicate@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-settle-duplicate@example.com')
        seller, _, product = self.create_store_product('seller-settle-duplicate@example.com', slug='store-settle-duplicate')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        order.status = ProductOrder.STATUS_COMPLETED
        order.completed_at = datetime.now(timezone.utc)
        ...
        first = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-dup-1', 'payout_address': 'seller-address-dup-1'},
            format='json',
        )
        self.assertEqual(first.status_code, status.HTTP_200_OK)
        second = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-dup-2', 'payout_address': 'seller-address-dup-2'},
            format='json',
        )
        self.assertEqual(second.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Only completed orders can be settled', second.data['detail'])
        payout = SellerPayout.objects.get(product_order=order)
        self.assertEqual(payout.txid, 'settle-dup-1')
        self.assertEqual(payout.payout_address, 'seller-address-dup-1')

    def test_seller_detail_shows_payout_summary_after_admin_settlement(self):
        admin = self.create_user('admin-settle-summary@example.com', is_staff=True, is_superuser=True)
        buyer = self.create_user('buyer-settle-summary@example.com')
        seller, _, product = self.create_store_product('seller-settle-summary@example.com', slug='store-settle-summary')
        address = self.create_shipping_address(buyer)
        order, _ = self.create_product_order(buyer, product, address)
        order.status = ProductOrder.STATUS_COMPLETED
        order.completed_at = datetime.now(timezone.utc)
        ...
        settle = self.client.post(
            reverse('admin-product-order-mark-settled', args=[order.order_no]),
            {'txid': 'settle-summary-1', 'payout_address': 'seller-summary-address', 'note': 'manual'},
            format='json',
        )
        self.assertEqual(settle.status_code, status.HTTP_200_OK)

        self.client.force_authenticate(user=seller)
        detail = self.client.get(reverse('seller-product-order-detail', args=[order.order_no]))
        self.assertEqual(detail.status_code, status.HTTP_200_OK)
        self.assertEqual(detail.data['status'], ProductOrder.STATUS_SETTLED)
        self.assertEqual(detail.data['payout']['status'], SellerPayout.STATUS_PAID)
        self.assertEqual(detail.data['payout']['txid'], 'settle-summary-1')
        self.assertEqual(detail.data['payout']['payout_address'], 'seller-summary-address')
...
class MembershipPaymentAssetAPITestCase(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='member-asset@example.com', password='pw123456')
        self.plan = MembershipPlan.objects.create(
            code=MembershipPlan.CODE_MONTHLY,
            name='Monthly',
            description='Monthly',
            price_lbc=Decimal('100.00000000'),
            duration_days=30,
            is_active=True,
        )
        self.client.force_authenticate(self.user)

    def test_plans_include_supported_payment_assets(self):
        ...
        self.assertIn('payment_asset_options', r.data[0])

    def test_payment_asset_rate_seed_exists(self):
        self.assertTrue(PaymentAssetRate.objects.filter(asset_code='thb_ltt').exists())
        self.assertTrue(PaymentAssetRate.objects.filter(asset_code='meow_points').exists())
        self.assertTrue(PaymentAssetRate.objects.filter(asset_code='meow_credit').exists())

    def test_payment_asset_rate_admin_registered(self):
        self.assertIn(PaymentAssetRate, admin.site._registry)

    def test_get_membership_rate_prefers_db_then_fallback_settings(self):
        PaymentAssetRate.objects.update_or_create(
            asset_code='meow_points',
            defaults={'display_name': 'MeowPoints', 'exchange_rate': Decimal('12'), 'is_active': True},
        )
        self.assertEqual(get_membership_payment_asset_rate('meow_points'), Decimal('12'))
        PaymentAssetRate.objects.filter(asset_code='meow_points').update(is_active=False)
        with override_settings(MEMBERSHIP_PAYMENT_ASSET_RATES={'meow_points': '7'}):
            self.assertEqual(get_membership_payment_asset_rate('meow_points'), Decimal('7'))

    def test_get_membership_rate_non_positive_raises(self):
        PaymentAssetRate.objects.update_or_create(
            asset_code='meow_credit',
            defaults={'display_name': 'MeowCredit', 'exchange_rate': Decimal('0'), 'is_active': True},
        )
        with self.assertRaises(Exception):
            get_membership_payment_asset_rate('meow_credit')
    ...
    def test_default_payment_asset_thb_ltt(self):
        ...
        self.assertEqual(r.data['payment_asset'], 'thb_ltt')
        self.assertEqual(r.data['payment_method'], 'blockchain')

    def test_meow_credit_success_and_balance_debit(self):
        PaymentAssetRate.objects.update_or_create(
            asset_code='meow_credit',
    ...
    def test_membership_me_active_after_platform_asset_paid(self):
        PaymentAssetRate.objects.update_or_create(
            asset_code='meow_points',
        ...
        self.assertEqual(r.status_code, 201)
        self.assertEqual(r.data['exchange_rate_snapshot'], '10.00000000')
        self.assertEqual(r.data['amount_snapshot'], '30.00000000')
        self.assertEqual(r.data['paid_amount'], '300.00000000')
    ...
    def test_plan_payment_asset_options_and_allow_flags(self):
        self.plan.base_price_amount = Decimal('30')
        self.plan.allow_meow_credit_payment = False
        self.plan.save(update_fields=['base_price_amount', 'allow_meow_credit_payment'])
        PaymentAssetRate.objects.update_or_create(asset_code='thb_ltt', defaults={'display_name': 'THB-LTT', 'exchange_rate': Decimal('1'), 'is_active': True})
        PaymentAssetRate.objects.update_or_create(asset_code='meow_points', defaults={'display_name': 'MeowPoints', 'exchange_rate': Decimal('10'), 'is_active': True})
        r = self.client.get(reverse('membership-plan-list'))
        self.assertEqual(r.status_code, 200)
        plan = r.data[0]
        self.assertEqual(plan['supported_payment_assets'], ['thb_ltt', 'meow_points'])
...
class WalletAPICompatibilityAPITestCase(APITestCase):
    def setUp(self):
    ...
    def test_membership_meow_points_payment_reduces_wallet_balance(self):
        plan = MembershipPlan.objects.create(
            ...
            name='Wallet Monthly',
            duration_days=30,
            price_lbc=Decimal('30.00000000'),
            base_price_amount=Decimal('30.00000000'),
            base_price_asset='thb_ltt',
            allow_blockchain_payment=True,
            allow_meow_points_payment=True,
            allow_meow_credit_payment=True,
            is_active=True,
        )
        PaymentAssetRate.objects.update_or_create(
            asset_code='meow_points',
        ...
        created = self.client.post(
            reverse('membership-order-create'),
            {'plan_code': plan.code, 'payment_asset': 'meow_points'},
            format='json',
        )
        self.assertEqual(created.status_code, status.HTTP_201_CREATED)
    ...
    def test_wallet_serializer_never_returns_null_balance(self):
        ...
        wallet.balance = None
        wallet.total_earned = None
        wallet.total_spent = None
